...
export function ManagersPage() {
  ...
  return (
    <div className="mx-auto max-w-5xl px-4 py-6">
      <div className="mb-5 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-ink-faint">
            To'lov tekshiruvchilar — har biriga pavilion/bloklar biriktiriladi
          </p>
        </div>
          ...
          <div key={m.id} className={`card p-4 ${!m.is_active ? "opacity-60" : ""}`}>
            ...
            <div className="mt-3 flex flex-wrap gap-1.5">
              ...
              <button
                ...
                onClick={() => {
                  if (confirm(`"${m.full_name ?? m.username}" managerini o'chirasizmi?`)) {
      ...
      {pavilionsFor && (
        <PavilionAssignModal manager={pavilionsFor} onClose={() => setPavilionsFor(null)} />
      )}

      {pwdFor && (
        <PasswordModal manager={pwdFor} onClose={() => setPwdFor(null)} />
      )}
    </div>
  );
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4">
      <div className="card w-full max-w-sm p-6">
        <div className="mb-3 flex items-center gap-2 text-status-paid">
          <CheckCircle2 size={20} />
          <h3 className="font-display text-lg font-bold text-ink">Manager yaratildi</h3>
        </div>
        <p className="mb-3 text-xs text-ink-faint">Login va parolni planshetga yozib bering — qayta ko'rsatilmaydi.</p>
...
function PasswordModal({ manager, onClose }: { manager: Manager; onClose: () => void }) {
  ...
  const mut = useMutation({
    mutationFn: () => changeManagerPassword(manager.id, pwd),
    onSuccess: onClose,
  });
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4" onClick={onClose}>
      <div className="card w-full max-w-sm p-5" onClick={(e) => e.stopPropagation()}>
        <div className="mb-3 flex items-center justify-between">
          <h3 className="font-display text-base font-bold text-ink">«{manager.full_name ?? manager.username}» parolini o'zgartirish</h3>
          <button onClick={onClose}><X size={18} className="text-ink-faint" /></button>
        </div>
        <input
          type="text"
        ...
        <button
          ...
          disabled={pwd.length < 6 || mut.isPending}
          onClick={() => mut.mutate()}
        >
...
function PavilionAssignModal({ manager, onClose }: { manager: Manager; onClose: () => void }) {
  ...
  const saveMut = useMutation({
    ...
    onSuccess: () => {
      qc.invalidateQueries({ queryKey: ["managers"] });
      qc.invalidateQueries({ queryKey: ["manager-pavilions", manager.id] });
      onClose();
    },
  });
        ...
        <div className="mb-3 flex items-center justify-between">
          <div>
            <h3 className="font-display text-lg font-bold text-ink">
              «{manager.full_name ?? manager.username}» — pavilionlar
            </h3>
