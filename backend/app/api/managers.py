...
class ManagerCreateOut(BaseModel):
    id: int
    username: str
    password: str  # faqat shu javobda ko'rsatiladi
...
class PavilionAssignIn(BaseModel):
    pavilion_ids: list[int]  # to'liq ro'yxat — shu manager uchun YAKUNIY holat
...
async def list_managers(
    ...
    """Shu bozordagi barcha managerlar ro'yxati."""
    ...
    for u in rows:
        cnt = await db.scalar(
            select(ManagerPavilion.id).where(ManagerPavilion.manager_id == u.id)
        )
        # cnt yuqoridagi scalar faqat bitta qiymat qaytaradi — count uchun alohida so'rov:
        from sqlalchemy import func as _func
        pcount = await db.scalar(
            select(_func.count(ManagerPavilion.id)).where(ManagerPavilion.manager_id == u.id)
        )
        out.append(ManagerOut(
...
async def create_manager(
    ...
    """Yangi manager yaratadi — login avtomatik, parol generatsiya qilinadi."""
...
async def toggle_manager_block(
    ...
    """Manager profilini bloklaydi/blokdan chiqaradi (is_active toggle)."""
    user = await db.scalar(
        select(User).where(User.id == manager_id, User.market_id == market.id, User.role == UserRole.MANAGER.value)
    )
    if user is None:
...
async def delete_manager(
    ...
    """Managerni butunlay o'chiradi (biriktirilgan pavilionlar ham o'chadi — CASCADE)."""
    user = await db.scalar(
        select(User).where(User.id == manager_id, User.market_id == market.id, User.role == UserRole.MANAGER.value)
    )
    if user is None:
...
async def get_manager_pavilions(
    ...
    """Shu bozordagi BARCHA pavilionlar — qaysilari shu managerga biriktirilgan
    belgisi bilan (UI checkbox grid uchun)."""
    user = await db.scalar(
        select(User).where(User.id == manager_id, User.market_id == market.id, User.role == UserRole.MANAGER.value)
    )
    if user is None:
...
async def assign_manager_pavilions(
    manager_id: int,
    body: PavilionAssignIn,
    _admin: AdminUser,
    market: CurrentMarket,
    db: Annotated[AsyncSession, Depends(get_db)],
) -> dict:
    """Manager uchun pavilion ro'yxatini to'liq almashtiradi (yakuniy holat).

    Eski biriktirishlar o'chiriladi, yangi ro'yxat yoziladi. Manager keyingi
    safar kirganida yangilangan ro'yxatni ko'radi.
    """
    user = await db.scalar(
        select(User).where(User.id == manager_id, User.market_id == market.id, User.role == UserRole.MANAGER.value)
    )
    ...
    # Faqat shu bozorga tegishli pavilionlarni qabul qilamiz (xavfsizlik)
    valid_ids = set((await db.execute(
        select(Pavilion.id).where(Pavilion.market_id == market.id, Pavilion.id.in_(body.pavilion_ids))
    )).scalars().all())
