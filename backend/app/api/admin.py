...
async def get_import_history(
    ...
    if category in ("rent", "all"):
        ...
        for rb in rb_list:
            ...
            fname = None
            imp_at = rb.updated_at.isoformat() if rb.updated_at else None
...
async def import_water(
    ...
    if touched_inns:
        existing = list((await db.execute(
            _select(MonthlyBalance).where(
                MonthlyBalance.year == year, MonthlyBalance.month == month,
                MonthlyBalance.category == "water", MonthlyBalance.inn.in_(touched_inns),
            )
        )).scalars())
        existing_map = {b.inn: b for b in existing}
        for inn in touched_inns:
            ...
            snapshot_rows.append({"key": key, "before": None if prev is None else {
                "inn": inn, "year": year, "month": month, "category": "water",
                "market_id": prev.market_id,
                "due_amount": str(prev.due_amount), "paid_amount": str(prev.paid_amount),
            }})
    ...
    try:
        ...
        snap = await save_snapshot(
            ...
            summary=f"Suv import: {year}-{month:02d} — {result.inns} INN, {result.with_debt} qarzli",
            audit_id=audit.id,
        )
        records = [{
            "inn": inn, "market_id": market.id, "year": year, "month": month,
        ...
        await db.rollback()
        raise HTTPException(status.HTTP_422_UNPROCESSABLE_ENTITY,
                            f"Bazaga saqlashda xatolik: {type(exc).__name__}: {exc}") from exc
...
async def upload_vacant_shops(
    ...
    Fayldagi shop_id lar is_vacant=True bo'ladi.
    Faylda bo'lmagan, lekin avval is_vacant=True bo'lgan do'konlar False bo'ladi.
    """
    ...
    # DB dan barcha do'konlarni yuklaymiz
    ...
    for s in all_shops:
        if s.shop_id in shop_ids_from_file:
            if not s.is_vacant:
                s.is_vacant = True
                s.inn = None  # bo'sh do'kon — INN tozalanadi
                marked_vacant += 1
        else:
            if s.is_vacant:
                s.is_vacant = False
    ...
    await write_audit(
        ...
        {"marked_vacant": marked_vacant, "marked_not_vacant": marked_not_vacant,
